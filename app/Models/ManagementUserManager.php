...
class ManagementUserManager extends Manager{
    ...
    public function allManagementUser(){
        $bdd = $this->bdConnect();

        $req = $bdd->query("SELECT *, DATE_FORMAT(dateinscription, '%d/%m/%Y') AS dateUserManager FROM users WHERE roleusers = 'user' ORDER BY id DESC");
    ...
    public function usersManagementEdition($id_users, $pseudo, $lastname, $firstname, $email, $roleusers){
        $bdd = $this->bdConnect();
